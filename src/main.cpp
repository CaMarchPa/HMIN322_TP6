...
int main(int argc, char const *argv[]) {
    ...
    // float Y_[width][width];
    // float Cr_[width][width];
    // float Cb_[width][width];
    ...
    for (int j = 0; j < height; j++) {
        for (int i = 0; i < width; i++) {
            ...
            // Y_[j][i] = 0.3 * img_read(i, j, 0, 0) + 0.6 * img_read(i, j, 0, 1) + 0.1 * img_read(i, j, 2);
            // Cb_[j][i] = -0.1687 * img_read(i, j, 0, 0) - 0.3313 * img_read(i, j, 0, 1) + 0.5 * img_read(i, j, 0, 2) + 128;
            // Cr_[j][i] = 0.5 * img_read(i, j, 0, 0) - 0.4187 * img_read(i, j, 0, 1) - 0.0813 * img_read(i, j, 0, 2) + 128;
        }
    }
    ...
    // float tab_[3][nbr_of_decomposition][width][width];
    // for (int j = 0; j < width; j++) {
    //     for (int i = 0; i < width; i++) {
    //         tab_[0][0][j][i] = Y_[j][i];
    //         tab_[1][0][j][i] = Cb_[j][i];
    //         tab_[2][0][j][i] = Cr_[j][i];
    //     }
    // }

    // for (int n = 1; n < nbr_of_decomposition; n++) {
    //     for (int j = 0; j < width; j++) {
    //         for (int i = 0; i < width; i++) {
    //             tab_[0][n][j][i] = 0.0f;
    //             tab_[1][n][j][i] = 0.0f;
    ...
    // std::cout << "step v1 : " << step << ", step v2 : " << log_2(height, nbr_of_decomposition) << '\n';
    ...
    for (int loop = 1; loop < nbr_of_decomposition; loop++) {
        ...
        for (int j = 0; j < matrix_size; j++)
            for (int i = 0; i < matrix_size; i++) {
                ...
                // tab_[0][loop][j][i] = tab_[0][loop - 1][j][i];
                // tab_[1][loop][j][i] = tab_[1][loop - 1][j][i];
                // tab_[2][loop][j][i] = tab_[2][loop - 1][j][i];
            }
        //(top, left)
        for (int j = 0; j < height; j+=2)
            for (int i = 0; i < height; i+=2) {
                ...
                // tab_[0][loop][j/2][i/2] = (tab_[0][loop-1][j][i] + tab_[0][loop-1][j+1][i] + tab_[0][loop-1][j][i+1] + tab_[0][loop-1][j+1][i+1]) / 4;
                // tab_[1][loop][j/2][i/2] = (tab_[1][loop-1][j][i] + tab_[1][loop-1][j+1][i] + tab_[1][loop-1][j][i+1] + tab_[1][loop-1][j+1][i+1]) / 4;
                // tab_[2][loop][j/2][i/2] = (tab_[2][loop-1][j][i] + tab_[2][loop-1][j+1][i] + tab_[2][loop-1][j][i+1] + tab_[2][loop-1][j+1][i+1]) / 4;
            }
        ...
        for (int j = 0; j < height; j+=2)
            for (int i = 0; i < height; i+=2) {
                ...
                // tab_[0][loop][j/2][half+i/2] = (tab_[0][loop-1][j][i] - tab_[0][loop-1][j][i+1]) / 2 + (tab_[0][loop-1][j+1][i] - tab_[0][loop-1][j+1][i+1]) / 2 + 128;
                // tab_[1][loop][j/2][half+i/2] = (tab_[1][loop-1][j][i] - tab_[1][loop-1][j][i+1]) / 2 + (tab_[1][loop-1][j+1][i] - tab_[1][loop-1][j+1][i+1]) / 2 + 128;
                // tab_[2][loop][j/2][half+i/2] = (tab_[2][loop-1][j][i] - tab_[2][loop-1][j][i+1]) / 2 + (tab_[2][loop-1][j+1][i] - tab_[2][loop-1][j+1][i+1]) / 2 + 128;
                ...
                // if (tab_[0][loop][j/2][half+i/2] > 128) {
                //     tab_[0][loop][j/2][half+i/2] = 128;
                // }
                // if (tab_[1][loop][j/2][half+i/2] > 128) {
                //     tab_[1][loop][j/2][half+i/2] = 128;
                // }
                // if (tab_[2][loop][j/2][half+i/2] > 128) {
                //     tab_[2][loop][j/2][half+i/2] = 128;
                // }
                score = 0.8;
        ...
        for (int j = 0; j < height; j+=2)
            for (int i = 0; i < height; i+=2) {
                ...
                // tab_[0][loop][half+j/2][i/2] = (tab_[0][loop-1][j][i] - tab_[0][loop-1][j][i+1]) / 2 + (tab_[0][loop-1][j+1][i] - tab_[0][loop-1][j+1][i+1]) / 2 + 128;
                // tab_[1][loop][half+j/2][i/2] = (tab_[1][loop-1][j][i] - tab_[1][loop-1][j][i+1]) / 2 + (tab_[1][loop-1][j+1][i] - tab_[1][loop-1][j+1][i+1]) / 2 + 128;
                // tab_[2][loop][half+j/2][i/2] = (tab_[2][loop-1][j][i] - tab_[2][loop-1][j][i+1]) / 2 + (tab_[2][loop-1][j+1][i] - tab_[2][loop-1][j+1][i+1]) / 2 + 128;
                ...
                // if (tab_[0][loop][half+j/2][i/2] > 128) {
                //     tab_[0][loop][half+j/2][i/2] = 128;
                // }
                // if (tab_[1][loop][half+j/2][i/2] > 128) {
                //     tab_[1][loop][half+j/2][i/2] = 128;
                // }
                // if (tab_[2][loop][half+j/2][i/2] > 128) {
                //     tab_[2][loop][half+j/2][i/2] = 128;
                // }
                score = 0.8;
                score *= (float)(nbr_of_decomposition + 4 - loop) / (float)(nbr_of_decomposition + 4);
        ...
        for (int j = 0; j < height; j+=2)
            for (int i = 0; i < height; i+=2) {
                ...
                // tab_[0][loop][half+j/2][half+i/2] = (tab_[0][loop-1][j][i] - tab_[0][loop-1][j+1][i] - tab_[0][loop-1][j][i+1] - tab_[0][loop-1][j+1][i+1]) / 2 + 128;
                // tab_[1][loop][half+j/2][half+i/2] = (tab_[1][loop-1][j][i] - tab_[1][loop-1][j+1][i] - tab_[1][loop-1][j][i+1] - tab_[1][loop-1][j+1][i+1]) / 2 + 128;
                // tab_[2][loop][half+j/2][half+i/2] = (tab_[2][loop-1][j][i] - tab_[2][loop-1][j+1][i] - tab_[2][loop-1][j][i+1] - tab_[2][loop-1][j+1][i+1]) / 2 + 128;
                ...
                // if (tab_[0][loop][half+j/2][half+i/2] > 128) {
                //     tab_[0][loop][half+j/2][half+i/2] = 128;
                // }
                // if (tab_[1][loop][half+j/2][half+i/2] > 128) {
                //     tab_[1][loop][half+j/2][half+i/2] = 128;
                // }
                // if (tab_[2][loop][half+j/2][half+i/2] > 128) {
                //     tab_[2][loop][half+j/2][half+i/2] = 128;
                // }
                score = 0.8;
                score *= (float)(nbr_of_decomposition + 4 - loop) / (float)(nbr_of_decomposition + 4);
    ...
    for (int j = 0; j < matrix_size; j++) {
        for (int i = 0; i < matrix_size; i++) {
            ...
            // tab_[0][nbr_of_decomposition-1][j][i] /= matrix_of_coef[j][i];
            // tab_[1][nbr_of_decomposition-1][j][i] /= matrix_of_coef[j][i];
            // tab_[2][nbr_of_decomposition-1][j][i] /= matrix_of_coef[j][i];
            if (tab_of_images[0][nbr_of_decomposition-1](i, j) < 10)
                compt++;
            // if (tab_[0][nbr_of_decomposition-1][j][i] < 10)
            //     compt++;
        }
    }

    char result[20] = "../img/result_.png";
    char result_Cr[25] = "../img/result_Cr.png";
    char result_Cb[25] = "../img/result_Cb.png";

    tab_of_images[0][nbr_of_decomposition - 1].save_png(result);
    ...
    // float tab_rec[3][nbr_of_decomposition][width][width];
    // for (int j = 0; j < width; j++) {
    //     for (int i = 0; i < width; i++) {
    //         tab_rec[0][nbr_of_decomposition-1][j][i] = tab_[0][nbr_of_decomposition-1][j][i];
    //         tab_rec[1][nbr_of_decomposition-1][j][i] = tab_[1][nbr_of_decomposition-1][j][i];
    //         tab_rec[2][nbr_of_decomposition-1][j][i] = tab_[2][nbr_of_decomposition-1][j][i];
    //     }
    // }
    ...
    for (int j = 0; j < matrix_size; j++) {
        for (int i = 0; i < matrix_size; i++) {
            ...
            // tab_rec[0][nbr_of_decomposition-1][j][i] *= matrix_of_coef[j][i];
            // tab_rec[1][nbr_of_decomposition-1][j][i] *= matrix_of_coef[j][i];
            // tab_rec[2][nbr_of_decomposition-1][j][i] *= matrix_of_coef[j][i];
        }
    }

    // for (int n = 0; n < nbr_of_decomposition-1; n++) {
    //     for (int j = 0; j < width; j++) {
    //         for (int i = 0; i < width; i++) {
    //             tab_rec[0][n][j][i] = 0.0f;
    //             tab_rec[1][n][j][i] = 0.0f;
    //             tab_rec[2][n][j][i] = 0.0f;
    //         }
    //     }
    // }
    ...
    for (int loop = nbr_of_decomposition - 2; loop >= 0; loop--) {
        ...
        for (int j = 0; j < height; j++) {
            for (int i = 0; i < height; i++) {
                ...
                // tab_rec[0][loop][2*j][2*i] = tab_rec[0][loop+1][j][i];
                // tab_rec[0][loop][2*j+1][2*i] = tab_rec[0][loop+1][j][i] + tab_rec[0][loop+1][j][i+half];
                // tab_rec[0][loop][2*j][2*i+1] = tab_rec[0][loop+1][j][i] + tab_rec[0][loop+1][j+half][i];
                // tab_rec[0][loop][2*j+1][2*i+1] = tab_rec[0][loop+1][j][i] + tab_rec[0][loop+1][j+half][i+half];
                //Cr
                ...
                // tab_rec[1][loop][2*j][2*i] = tab_rec[1][loop+1][j][i];
                // tab_rec[1][loop][2*j+1][2*i] = tab_rec[1][loop+1][j][i] + tab_rec[1][loop+1][j][i+half];
                // tab_rec[1][loop][2*j][2*i+1] = tab_rec[1][loop+1][j][i] + tab_rec[1][loop+1][j+half][i];
                // tab_rec[1][loop][2*j+1][2*i+1] = tab_rec[1][loop+1][j][i] + tab_rec[1][loop+1][j+half][i+half];
                //Cb
                ...
                // tab_rec[2][loop][2*j][2*i] = tab_rec[2][loop+1][j][i];
                // tab_rec[2][loop][2*j+1][2*i] = tab_rec[2][loop+1][j][i] + tab_rec[2][loop+1][j][i+half];
                // tab_rec[2][loop][2*j][2*i+1] = tab_rec[2][loop+1][j][i] + tab_rec[2][loop+1][j+half][i];
                // tab_rec[2][loop][2*j+1][2*i+1] = tab_rec[2][loop+1][j][i] + tab_rec[2][loop+1][j+half][i+half];
            }
        }
    }
    ...
    for (int j = 0; j < matrix_size; j++) {
        for (int i = 0; i < matrix_size; i++) {
            ...
            // result_rec_img(i, j, 0, 1) = tab_of_images_rec[0][0](i, j) - 0.34414 * (tab_of_images_rec[2][0](i, j) - 128) - 0.71414 * (tab_of_images_rec[1][0](i, j) - 128);
            // result_rec_img(i, j, 0, 2) = tab_of_images_rec[0][0](i, j) + 1.772 * (tab_of_images_rec[2][0](i, j) - 128);
    ...
    CImg<> rgb = result_rec_img.get_YCbCrtoRGB();
    rgb.save_png(result_f);
    double psnr = img_read.PSNR(rgb);
    std::cout << "PSNR = " << psnr << '\n';
